...
public class AccountLookupController {
    ...
    public ResponseEntity<AccountLookupResponse> getAllAccounts() {
        try {
            ...
            var response = AccountLookupResponse.builder()
                    .accounts(accounts)
                    .message(MessageFormat.format("Successfully returned {0} account(s)."
                            ,accounts.size()))
                    .build();
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (Exception ex) {
            var safeError = "Fail to get all accounts request!";
            logger.log(Level.SEVERE, safeError, ex);
            return new ResponseEntity<>(new AccountLookupResponse(safeError),
    ...
    public ResponseEntity<AccountLookupResponse> getAccountById(@PathVariable("id") String id) {
        try {
            ...
            var safeError = "Fail to get account by Id request!";
            logger.log(Level.SEVERE, safeError, ex);
            return new ResponseEntity<>(new AccountLookupResponse(safeError),
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<AccountLookupResponse> getAccountByHolder(
        ...
        try {
            ...
            var safeError = "Fail to get account by holder request!";
            logger.log(Level.SEVERE, safeError, ex);
            return new ResponseEntity<>(new AccountLookupResponse(safeError),
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<AccountLookupResponse> getAccountWithBlance(
            @PathVariable("equalityType") EqualityType equalityType,
            @PathVariable("balance") double balance) {
        try {
            List<BankAccount> accounts = queryDispatcher.send(
                    new FindAccountWithBalanceQuery(equalityType, balance));
            if (accounts == null || accounts.size() == 0) {
                return new ResponseEntity<>(null, HttpStatus.NO_CONTENT);
            }
            var response = AccountLookupResponse.builder()
            ...
            var safeError = "Fail to get account with balance request!";
            logger.log(Level.SEVERE, safeError, ex);
            return new ResponseEntity<>(new AccountLookupResponse(safeError),
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
