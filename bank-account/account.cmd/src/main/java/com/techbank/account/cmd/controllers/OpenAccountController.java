    ...
    public ResponseEntity<BaseResponse> openAccount(@RequestBody OpenAccountCommand command) {
        ...
        try {
            ...
            return new ResponseEntity<>(
                    new OpenAccountResponse("Creation completed successfully.", id),
                    HttpStatus.CREATED);
            ...
            var safeError = MessageFormat.format("An error was occured during processing to " +
                    "open a new bank account for id - {0}", id);
            logger.log(Level.SEVERE, safeError, ex);
            return new ResponseEntity<>(new OpenAccountResponse(safeError, id),
